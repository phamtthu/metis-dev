...
export class SequenceService {
    ...
    async getList(paginateQuery: PaginationQueryDto, search: string) {
        try {
            ...
            if (paginateQuery.offset >= 0 && paginateQuery.limit >= 0) {
                ...
                return await this.sequenceModel.find(query)
                    .sort({ 'created_at': SortQuery.Desc })
        } catch (error) { throwSrvErr(error) }
    }
    ...
    async delete(sequenceId: string) {
        try {
            const deletedSequence = await this.sequenceModel.findByIdAndDelete(sequenceId)
    ...
    async update(sequenceId: string, sequenceDTO: UpdateSequenceDTO) {
        try {
            const oldSequence = await this.sequenceModel
                .findById(sequenceId).lean()
            // Process
            ...
            // Resource
            if (!isTwoArrayEqual(sequenceDTO.resources, oldSequence.resources.map(e => String(e)))) {
                await this.resourceModel.updateMany(
            ...
            // User
            if (!isTwoArrayEqual(sequenceDTO.users, oldSequence.users.map(e => String(e)))) {
                await this.userModel.updateMany(
