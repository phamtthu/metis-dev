...
export class ProcessService {
    ...
    async getList(paginateQuery: PaginationQueryDto, search: string) {
        try {
            ...
            if (paginateQuery.offset >= 0 && paginateQuery.limit >= 0) {
                ...
                return await this.processModel.find(query)
                    .sort({ 'created_at': SortQuery.Desc }).select('username')
        } catch (error) { throwSrvErr(error) }
    }
    ...
    async getDetail(processId: string) {
        try {
            ...
            const sequences = await this.sequenceModel.find({ process: processId })
                .populate(sequencePopulateOption).lean()
    ...
    async delete(processId: string) {
        try {
            const deletedProcess = await this.processModel.findByIdAndDelete(processId)
            // Sequence
    ...
    async update(processId: string, processDTO: UpdateProcessDTO) {
        try {
            return await this.processModel.findByIdAndUpdate(
                processId,
                processDTO,
                { new: true })
        } catch (error) { throwSrvErr(error) }
