...
export class TaskService {
    ...
    async create(taskDTO: AddTaskDTO, originURL: string) {
        try {
            ...
            if (result)
                throw new ConflictException('task_no is already exist')

            taskDTO.images = await Promise.all(taskDTO.images.map(async (img) =>
                await getNewImgLink(img, 'task', originURL)))
