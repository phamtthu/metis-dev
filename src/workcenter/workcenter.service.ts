    ...
    try {
      ...
      if (wcResources.length > 0)
        throw new BadRequestException(
          'Can not delete WorkCenter. There are Resource link with this WorkCenter',
        );
      const wcUsers = await this.wcUserModel.find({ workcenter: workCenterId });
      if (wcUsers.length > 0)
        throw new BadRequestException(
          'Can not delete WorkCenter. There are User link with this WorkCenter',
        );
    ...
    try {
      ...
      if (Array.isArray(workCenterDTO.resources)) {
        ...
        const workCenterResources = workCenterDTO.resources.map(
          (resourceId) => ({ workcenter: workCenterId, resource: resourceId }),
        );
