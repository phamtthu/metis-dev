...
export class ResourceCategoryService {
  constructor(
    ...
    private rCategoryModel: PaginateModel<ResourceCategory>,
    ...
    try {
      ...
      if (search) {
        ...
        if (paginateQuery.offset >= 0 && paginateQuery.limit >= 0) {
          ...
          return await this.rCategoryModel
            .find(query)
            .sort({ created_at: SortQuery.Desc });
        }
      } else {
        const categories = await this.rCategoryModel
          .find()
          .sort({ created_at: SortQuery.Desc })
          .lean();
  ...
  async getDetail(rCategoryId: string) {
    try {
      const theCategory: any = await this.rCategoryModel
        .findById(rCategoryId)
        .lean();
      const categories: any = await this.rCategoryModel
        .find()
        .sort({ created_at: SortQuery.Desc })
        .lean();
  ...
  async delete(rCategoryId: string) {
    const deletedRCategory = await this.rCategoryModel.findByIdAndDelete(
      rCategoryId,
    );
    ...
    await this.rCategoryModel.updateMany(
      { parent: rCategoryId },
      { parent: null },
    );
