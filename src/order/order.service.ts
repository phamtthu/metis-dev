    ...
    async update(orderId: string, orderDTO: UpdateOrderDTO) {
        try {
            ...
            if (!isTwoArrayEqual(oldProductIds, newProductIds)) {
                const removedProducts = oldProductIds.filter((productId) =>
                    !newProductIds.includes(String(productId)))

                const incomingProducts = newProductIds.filter((productId) =>
                    !oldProductIds.includes(String(productId)))
